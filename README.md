...
Posteroom modifications began on 2026-09-02. The current version is `0.4.2-posteroom.13` (WordPress plugin `0.1.5`), adding a compact Full HD desktop layout with reserved space for the map controls while retaining the resilient, same-origin guest WooCommerce cart flow. Git history records the modified files and dates.

...
WooCommerce integration, restricted paper sizes and the purchase flow are not implemented yet. The app is not ready to mount in WordPress: its global CSS, root-relative assets and root-scoped service worker must be adapted first. Do not copy the standalone service worker into the WordPress root.
...
Posteroom modifications began on 2026-09-02. The first branding changes are versioned as `0.4.2-posteroom.1`. Git history records the modified files and dates.
